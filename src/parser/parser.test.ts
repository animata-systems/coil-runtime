...
END`;
    ...
    expect(node.for).toHaveLength(1);
    expect(node.for[0].ref).toEqual({ kind: 'literal', value: 'user' });
  ...
  it('SEND с несколькими получателями FOR @a, @b', () => {
    ...
    const ast = parseEN(src);
    const node = ast.nodes[0] as SendNode;
    expect(node.for).toHaveLength(2);
    expect(node.for[0].ref).toEqual({ kind: 'literal', value: 'alice' });
    expect(node.for[1].ref).toEqual({ kind: 'literal', value: 'bob' });
  });
...
EXIT`;
    ...
    expect(think.using[0].ref).toEqual({ kind: 'literal', value: 'search' });
...
EXIT`;
    ...
    expect(think.using[0].ref).toEqual({ kind: 'literal', value: 'search' });
    expect(think.using[1].ref).toEqual({ kind: 'literal', value: 'calculator' });
  });
...
EXIT`;
    ...
    expect(exec.tool.ref).toEqual({ kind: 'literal', value: 'load_article' });
...
ВЫХОД`;
    ...
    expect(exec.tool.ref).toEqual({ kind: 'literal', value: 'поиск' });
