...
**Почему.** Дублирование Comma-separated разбора в 4 местах — нарушение DRY с одинаковой логикой. Generic-функция — один источник истины для паттерна «список ссылок через запятую».

**Цена.** Return type зависит от параметра — нужна либо перегрузка, либо discriminated union на выходе. Минимальная сложность, оправданная устранением дублирования.
...
| **Статус** | принят |
| **Дата** | 2026-03-26 |
| **Scope** | `src/parser/parser.ts`, `cli/index.ts`, `src/browser.ts` |
...
**Почему.** Альтернатива — реконструировать текст из token values — теряет пробелы и форматирование, не соответствует принципу «сырой текст до конца строки». Source — единственный надёжный источник.

**Цена.** Один дополнительный параметр в публичном API `parse()`. Все вызовы (CLI, IDE, тесты) обновляются тривиально — source уже доступен в каждом из них (это та же строка, что передаётся в `tokenize()`).
...
**Почему.** Разделение синтаксис/семантика (R-0007): парсер проверяет типы токенов, валидатор — имена и объявления. Тип токена ($ vs ?) определяется лексером и является синтаксическим свойством, не семантическим.

**Цена.** Тест `wait-value-not-promise.coil` переходит в scope STORY-006 (был «семантический, вне scope»).
...
| **Статус** | принят |
| **Дата** | 2026-03-26 |
| **Scope** | `src/ast/nodes.ts`, `src/parser/parser.ts` |
...
**Почему.** Discriminated union — идиоматичный TypeScript. Единообразие с `TemplatePart.type`, `OperatorNode.kind`. Добавлено до появления потребителей — нулевая миграция.

**Цена.** Каждое создание literal/ref в парсере получает дополнительное поле. Минимальная.

## R-0022 — Known gaps фазы 1 (STORY-006)

| | |
|---|---|
| **Статус** | принят |
| **Дата** | 2026-03-26 |
| **Scope** | `src/parser/parser.ts`, `src/lexer/tokenizer.ts` |

**Контекст.** Код-ревью фазы 1 выявило ряд замечаний, которые не блокируют текущую фазу, но требуют внимания в будущем.

**Известные ограничения:**

1. **`readNumber()` — только целые числа.** Дробные литералы (3.14) не поддерживаются. В тестовом корпусе спеки только целые. Расширение — при появлении потребности.
2. **Comparison operators (`>`, `>=`, `<`, `<=`, `=`, `!=`) токенизируются как `Identifier`.** Рабочее решение для D-006-1 (условия — raw text). При реализации expression parser потребуется отдельный тип токена.
3. **`WAIT ON` — обязательность не проверяется парсером.** `WAIT END` без `ON` создаёт `WaitNode { on: [] }` без ошибки. Обязательность `ON` — семантическая проверка, scope валидатора (R-0007).
4. **Silent skip неизвестных токенов** в `parseThink`, `parseWait`, `parseSend`. Tolerant parsing — осознанный выбор для tooling. Diagnostic warning для неизвестных токенов — задача фазы 6.
5. **`expect()` использует `skipTrivia()` внутри**, что позволяет комментарии между элементами arg list и RESULT. Корректно по R-0015. Ужесточение при необходимости — отдельная задача.
